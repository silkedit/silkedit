...
DEPRECATED(RUBY_EXTERN struct rb_blocking_region_buffer *
               rb_thread_blocking_region_begin(void));
DEPRECATED(RUBY_EXTERN void rb_thread_blocking_region_end(
    struct rb_blocking_region_buffer *));
#define TRAP_BEG                                                               \
  do {                                                                         \
...
#define TRAP_END                                                               \
  rb_thread_blocking_region_end(__region);                                     \
  }                                                                            \
  while (0)

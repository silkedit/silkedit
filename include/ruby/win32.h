...
extern int WSAAPI
    rb_w32_select(int, fd_set *, fd_set *, fd_set *, struct timeval *);
...
extern int WSAAPI
    rb_w32_recvfrom(int, char *, int, int, struct sockaddr *, int *);
extern int WSAAPI rb_w32_send(int, const char *, int, int);
extern int WSAAPI
    rb_w32_sendto(int, const char *, int, int, const struct sockaddr *, int);
...
static inline float rb_infinity_float(void) { return INFINITY; }
#pragma warning(pop)
#undef INFINITY
...
#endif
static inline double scalb(double a, long b) { return _scalb(a, b); }
#else
...
long rb_w32_write_console(
    uintptr_t,
    int); /* use uintptr_t instead of VALUE because it's not defined yet here */
...
*/
typedef uintptr_t (*asynchronous_func_t)(uintptr_t self, int argc,
                                         uintptr_t *argv);
 ...
 */
static inline double rb_w32_pow(double x, double y) { return powl(x, y); }

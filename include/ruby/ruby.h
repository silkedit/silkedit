...
typedef char
    ruby_check_sizeof_long_long[SIZEOF_LONG_LONG == sizeof(LONG_LONG) ? 1 : -1];
#endif
...
#define SYMBOL_P(x)                                                            \
  (((VALUE)(x) & ~((~(VALUE)0) << RUBY_SPECIAL_SHIFT)) == SYMBOL_FLAG)
...
enum ruby_special_consts {
  ...
  RUBY_FIXNUM_FLAG = 0x01,
  RUBY_FLONUM_MASK
  = 0x00, /* any values ANDed with FLONUM_MASK cannot be FLONUM_FLAG */
  RUBY_FLONUM_FLAG = 0x02,
...
#define RB_GC_GUARD_PTR(ptr)                                                   \
  __extension__({                                                              \
    volatile VALUE *rb_gc_guarded_ptr = (ptr);                                 \
    rb_gc_guarded_ptr;                                                         \
  })
#else
#ifdef _MSC_VER
#pragma optimize("", off)
static inline volatile VALUE *rb_gc_guarded_ptr(volatile VALUE *ptr) {
  return ptr;
}
#pragma optimize("", on)
#else
...
int ruby_safe_level_4_warning(void)
    __attribute__((warning("$SAFE=4 is obsolete")));
#ifdef RUBY_EXPORT
...
static inline LONG_LONG rb_num2ll_inline(VALUE x) {
  if (FIXNUM_P(x))
    return FIX2LONG(x);
  else
    return rb_num2ll(x);
...
#define CLONESETUP(clone, obj)                                                 \
  do {                                                                         \
    OBJSETUP((clone), rb_singleton_class_clone((VALUE)(obj)),                  \
             RBASIC(obj)->flags);                                              \
...
#define DUPSETUP(dup, obj)                                                     \
  do {                                                                         \
    OBJSETUP((dup), rb_obj_class(obj),                                         \
             (RBASIC(obj)->flags) & (T_MASK | FL_EXIVAR | FL_TAINT));          \
    if (FL_TEST((obj), FL_EXIVAR))                                             \
      rb_copy_generic_ivar((VALUE)(dup), (VALUE)(obj));                        \
  } while (0)
...
struct RObject {
  ...
  union {
    struct {
      long numiv;
      VALUE *ivptr;
      struct st_table *iv_index_tbl; /* shortcut for
...
#define ROBJECT_NUMIV(o)                                                       \
  ((RBASIC(o)->flags & ROBJECT_EMBED) ? ROBJECT_EMBED_LEN_MAX                  \
                                      : ROBJECT(o)->as.heap.numiv)
#define ROBJECT_IVPTR(o)                                                       \
  ((RBASIC(o)->flags & ROBJECT_EMBED) ? ROBJECT(o)->as.ary                     \
                                      : ROBJECT(o)->as.heap.ivptr)
#define ROBJECT_IV_INDEX_TBL(o)                                                \
  ((RBASIC(o)->flags & ROBJECT_EMBED) ? RCLASS_IV_INDEX_TBL(rb_obj_class(o))   \
...
#define RSTRING_LEN(str)                                                       \
  (!(RBASIC(str)->flags & RSTRING_NOEMBED) ? RSTRING_EMBED_LEN(str)            \
                                           : RSTRING(str)->as.heap.len)
#define RSTRING_PTR(str)                                                       \
  (!(RBASIC(str)->flags & RSTRING_NOEMBED) ? RSTRING(str)->as.ary              \
...
#define RSTRING_GETMEM(str, ptrvar, lenvar)                                    \
  (!(RBASIC(str)->flags & RSTRING_NOEMBED)                                     \
       ...
       : ((ptrvar) = RSTRING(str)->as.heap.ptr,                                \
          (lenvar) = RSTRING(str)->as.heap.len))
...
#define RARRAY_PTR(a)                                                          \
  ((VALUE *)RARRAY_CONST_PTR(                                                  \
      RGENGC_WB_PROTECTED_ARRAY ? OBJ_WB_UNPROTECT((VALUE)a) : ((VALUE)a)))
...
#define RHASH_SIZE(h)                                                          \
  (RHASH(h)->ntbl ? (st_index_t)RHASH(h)->ntbl->num_entries : 0)
...
#define Data_Wrap_Struct(klass, mark, free, sval)                              \
  rb_data_object_alloc((klass), (sval), (RUBY_DATA_FUNC)(mark),                \
                       (RUBY_DATA_FUNC)(free))
...
#define RSTRUCT_CONST_PTR(st)                                                  \
  ((RBASIC(st)->flags & RSTRUCT_EMBED_LEN_MASK) ? RSTRUCT(st)->as.ary          \
                                                : RSTRUCT(st)->as.heap.ptr)
#define RSTRUCT_PTR(st)                                                        \
  ((VALUE *)RSTRUCT_CONST_PTR(                                                 \
      RGENGC_WB_PROTECTED_STRUCT ? OBJ_WB_UNPROTECT((VALUE)st) : (VALUE)st))
...
#if (SIZEOF_VALUE * 3 / SIZEOF_ACTUAL_BDIGIT)                                  \
    < (1 << RBIGNUM_EMBED_LEN_NUMBITS) - 1
...
#define RBIGNUM_DIGITS(b)                                                      \
  ((RBASIC(b)->flags & RBIGNUM_EMBED_FLAG) ? RBIGNUM(b)->as.ary                \
...
#define OBJ_FROZEN(x)                                                          \
  (!!(FL_ABLE(x) ? (RBASIC(x)->flags & (FL_FREEZE))                            \
                 : (FIXNUM_P(x) || FLONUM_P(x) || SYMBOL_P(x))))
#define OBJ_FREEZE(x) FL_SET((x), FL_FREEZE)

#if USE_RGENGC
#define OBJ_PROMOTED(x) (SPECIAL_CONST_P(x) ? 0 : FL_TEST_RAW((x), FL_PROMOTED))
#define OBJ_WB_PROTECTED(x)                                                    \
  (SPECIAL_CONST_P(x) ? 1 : FL_TEST_RAW((x), FL_WB_PROTECTED))
...
static inline VALUE rb_obj_written(VALUE a, RB_UNUSED_VAR(VALUE oldv), VALUE b,
                                   RB_UNUSED_VAR(const char *filename),
                                   RB_UNUSED_VAR(int line)) {
...
#else
static inline VALUE rb_int2num_inline(int v) {
  if (FIXABLE(v))
    return INT2FIX(v);
  else
    return rb_int2big(v);
...
static inline VALUE rb_uint2num_inline(unsigned int v) {
  if (POSFIXABLE(v))
    return LONG2FIX(v);
  else
...
#else
#define ALLOCV(v, n)                                                           \
  ((n) < 1024 ? (RB_GC_GUARD(v) = 0, alloca(n))                                \
...
void rb_define_virtual_variable(const char *, VALUE (*)(ANYARGS),
                                void (*)(ANYARGS));
void rb_define_hooked_variable(const char *, VALUE *, VALUE (*)(ANYARGS),
                               void (*)(ANYARGS));
...
#define CONST_ID(var, str)                                                     \
  do                                                                           \
  CONST_ID_CACHE((var) =, (str)) while (0)
#ifdef __GNUC__
/* __builtin_constant_p and statement expression is available
 * since gcc-2.7.2.3 at least. */
#define rb_intern(str)                                                         \
  (__builtin_constant_p(str) ? __extension__(CONST_ID_CACHE((ID), (str)))      \
                             : rb_intern(str))
...
PRINTF_ARGS(void rb_compile_warning(const char *, int, const char *, ...), 3,
            4);
...
#define RB_BLOCK_CALL_FUNC_ARGLIST(yielded_arg, callback_arg)                  \
  VALUE yielded_arg, VALUE callback_arg, int argc, const VALUE *argv,          \
      VALUE blockarg
typedef VALUE rb_block_call_func(RB_BLOCK_CALL_FUNC_ARGLIST(yielded_arg,
                                                            callback_arg));
...
#define RB_FLOAT_TYPE_P(obj)                                                   \
  (FLONUM_P(obj) || (!SPECIAL_CONST_P(obj) && BUILTIN_TYPE(obj) == T_FLOAT))
...
#define rb_special_const_p(obj)                                                \
  __extension__({                                                              \
    ...
    (int)(SPECIAL_CONST_P(special_const_obj) ? Qtrue : Qfalse);                \
  })
#else
...
#define RUBY_EVENT_SWITCH                                                      \
  0x040000 /* obsolete name. this macro is for compatibility */
           /* 0x080000 */

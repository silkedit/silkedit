...
msgpack::rpc::msgid_t PluginManagerPrivate::s_msgId = 0;
std::unordered_map<msgpack::rpc::msgid_t, ResponseResult*> PluginManagerPrivate::s_eventLoopMap;
...
PluginManagerPrivate::PluginManagerPrivate(PluginManager* q_ptr)
    : q(q_ptr), m_pluginProcess(nullptr), m_socket(nullptr), m_server(nullptr), m_isStopped(false) {
...
void PluginManagerPrivate::pluginRunnerConnected() {
  ...
  connect(m_socket, &QLocalSocket::readyRead, this, &PluginManagerPrivate::readRequest,
          Qt::QueuedConnection);
  connect(m_socket,

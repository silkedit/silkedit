...
// init msgpack-rpc client
const client = rpc.createClient(socketFile, () => {
...
const handler = {
  ...
  ,"translate": (key, defaultText, response) => {
    response.result(t(key, defaultText))
  }
          ...
          try {
            ...
            if (module.commands) {
              if (pjson.name === 'default') {
                ...
                unregisterCommands(Object.keys(module.commands));
              } else {
                for (var prop in module.commands) {
                  delete commands[pjson.name + '.' + prop]
                }
                unregisterCommands(Object.keys(module.commands).map(c => pjson.name + '.' + c));
              }
            } else {

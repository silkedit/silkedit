...
const bridge = require('./silkeditbridge')
...
bridge.JSHandler.askCondition = (name, operator, value) => {
    if (name in conditions) {
        try {
          return conditions[name](operator, value)
        } catch (err) {
          console.error(err)
          return false
        }
...
bridge.JSHandler.keyEventFilter = (type, key, repeat, altKey, ctrlKey, metaKey, shiftKey, response) => {
    ...
    if (type in eventFilters) {
        try {
          response.setResult(eventFilters[type].some((fn) => { return fn(event)}))
        } catch (err) {
          console.error(err)
          ...
          try {
            ...
            if (module.commands) {
              if (pjson.name === 'default') {
                ...
                bridge.API.unregisterCommands(Object.keys(module.commands));
              } else {
                for (var prop in module.commands) {
                  delete commands[pjson.name + '.' + prop]
                }
                bridge.API.unregisterCommands(Object.keys(module.commands).map(c => pjson.name + '.' + c));
              }
            } else {
  ...
  fs.readdir(dir, (err, files) => {
    ...
    fs.open(packageJsonPath, 'r', (err, fd) => {
      fd && fs.close(fd, (err) => {
        ...
        fs.open(keymapPath, 'r', (err, fd) => {
          fd && fs.close(fd, (err) => {
            bridge.API.loadKeymap(pjson.name, keymapPath);
          })
        })
        ...
        fs.open(menuFilePath, 'r', (err, fd) => {
          fd && fs.close(fd, (err) => {
            bridge.API.loadMenu(pjson.name, menuFilePath);
          })
        })
        ...
        fs.open(toolbarFilePath, 'r', (err, fd) => {
          fd && fs.close(fd, (err) => {
            bridge.API.loadToolbar(pjson.name, toolbarFilePath);
          })
        })
        ...
        fs.open(configPath, 'r', (err, fd) => {
          if (fd) {
            try {
              if (pjson.name != 'default') {
                bridge.API.loadConfig(pjson.name, configPath)
              }
          ...
          if (pjson.main) {
            ...
            if (module.commands) {
              if (pjson.name === 'default') {
                ...
                bridge.API.registerCommands(Object.keys(module.commands).map((cmd) => {
...
customKeymapManager.dispatch = (keyEvent) => {
  if (keyEvent != null) {
    bridge.KeymapManager.dispatchFromJS(keyEvent.type, keyEvent.key, keyEvent.repeat, keyEvent.altKey, keyEvent.ctrlKey, keyEvent.metaKey, keyEvent.shiftKey)
  }
}
...
const silkedit = {
    ...
    registerCondition: (name, func) => {
      ...
      bridge.API.registerCondition(name)
    },

    unregisterCondition: (name) => {
      delete conditions[name]
      bridge.API.unregisterCondition(name)
    },

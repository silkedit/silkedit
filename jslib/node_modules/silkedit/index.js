...
function unloadPackage(pjson, module) {
  if (module.commands) {
    if (pjson.name === 'default') {
      ...
      Object.keys(module.commands).map(c => pjson.name + '.' + c).forEach(cmd => CommandManager.remove(cmd));
    }
  }
...
function deactivatePackages() {
  loadedModules.forEach(module => {
    if (module.deactivate) module.deactivate()
  });
}
...
bridge.JSHandler.deactivatePackages = deactivatePackages;
...
bridge.JSHandler.runCommand = CommandManager.run;
...
function tr(key, packageName, defaultValue) {
  ...
  if (packageName in packageDirMap) {
    try {
      return bridge.translate(packageDirMap[packageName], key, defaultValue);
    } catch(e) {
      console.warn(e);
    }
  } else {

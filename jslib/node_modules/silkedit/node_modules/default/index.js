...
module.exports = {
    ...
    silkedit.ConditionManager.add('find_replace_view_visible', findReplaceViewVisibleCond);
	},
    ...
    silkedit.ConditionManager.remove('find_replace_view_visible');
	},

	commands: {
		"new_file": () => {
      if (silkedit.Window.windows().length == 0) {
        const win = silkedit.Window.createWithNewFile();
        if (win == null) {
        ...
        return;
			...
			let tabView = silkedit.App.activeTabView();
			if (tabView == null) {
    ...
    "move_cursor_end_of_line": (args) => {
      ...
      fs.readFile(userRootPackageJsonPath, 'utf-8', (err, data) => {
          ...
          try {
            ...
            const validate = (input, pos) => {
              ...
              if (result.validForNewPackages === true) {
                if (!pkgnameSet.has(input)) {
                  return {
                    state: silkedit.Validator.State.Acceptable
                  };
                } else {
                  return {
                    state: silkedit.Validator.State.Intermediate
                  };
                }
              } else {
                return {
                  state: silkedit.Validator.State.Invalid
                };
              }
            }
            validator = new silkedit.Validator(validate);
          } catch(err) {
            console.error(err)
            validator = null
          }
        ...
        const dialog = new InputDialog(silkedit.tr("enter_new_package_name", 'default', "Enter new package name"), "my_package", validator);

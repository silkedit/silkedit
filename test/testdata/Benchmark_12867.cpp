...
QWidgetBackingStoreTracker::QWidgetBackingStoreTracker() : m_ptr(0) {}
...
void QWidgetPrivate::adjustFlags(Qt::WindowFlags& flags, QWidget* w) {
  bool customize = (flags & (Qt::CustomizeWindowHint | Qt::FramelessWindowHint |
...
void QWidgetPrivate::init(QWidget* parentWidget, Qt::WindowFlags f) {
  ...
  // give potential windows a bigger "pre-initial" size; create_sys() will give them a new size
  // later
  ...
  if (QApplicationPrivate::testAttribute(
          Qt::AA_ImmediateWidgetCreation))  // ### fixme: Qt 6: Remove AA_ImmediateWidgetCreation.
    q->create();
  ...
  if (parentWidget && parentWidget->d_func() && parentWidget->d_func()->isInUnifiedToolbar) {
    if (parentWidget->d_func()->unifiedSurface) {
      ...
      parentWidget->d_func()->unifiedSurface->recursiveRedirect(toolbar, toolbar,
                                                                toolbar->d_func()->toolbar_offset);
    }
  }
#endif  // Q_DEAD_CODE_FROM_QT4_MAC
}
...
void QWidget::create(WId window, bool initializeWindow, bool destroyOldWindow) {
...
#ifdef ALIEN_DEBUG
  qDebug() << "QWidget::create:" << this << "parent:" << parentWidget() << "Alien?"
           << !testAttribute(Qt::WA_NativeWindow);
#endif
  ...
  if (testAttribute(Qt::WA_WState_Created) && !internalWinId() &&
      testAttribute(Qt::WA_NativeWindow) && d->extra && d->extra->dropTarget) {
        ...
        if (childWidget->testAttribute(Qt::WA_NativeWindow)) {
          ...
          if (childWidget->windowHandle()) {
            if (childWidget->isWindow()) {
              childWidget->windowHandle()->setTransientParent(
                  parentWidget->window()->windowHandle());
            } else {
              childWidget->windowHandle()->setParent(
                  childWidget->nativeParentWidget()->windowHandle());
            }
          }
        } else {
...
void QWidgetPrivate::create_sys(WId window, bool initializeWindow, bool destroyOldWindow) {
  ...
  if (win->isTopLevel()) {
    ...
    if (screenNumber < 0) {
      screenNumber = q->windowType() != Qt::Desktop ? QApplication::desktop()->screenNumber(q) : 0;
    }
  ...
  qt_window_private(win)->positionPolicy = topData()->posIncludesFrame
                                               ...
                                               : QWindowPrivate::WindowFrameExclusive;
  win->create();
...
void QWidgetPrivate::createTLSysExtra() {
  ...
  if (!extra->topextra->window && (q->testAttribute(Qt::WA_NativeWindow) || q->isWindow())) {
    ...
    if (activeXNativeParentHandle.isValid())
      extra->topextra->window->setProperty(activeXNativeParentHandleProperty,
                                           activeXNativeParentHandle);
...
QWidget::~QWidget() {
  ...
  QT_TRY {
    ...
    if (!e || !e->proxyWidget ||
        (w->parentWidget() && w->parentWidget()->d_func()->focus_child == this))
#endif
      clearFocus();
  }
  QT_CATCH(...) {
  ...
  if (isWindow() && isVisible() && internalWinId()) {
    QT_TRY { d->close_helper(QWidgetPrivate::CloseNoEvent); }
    QT_CATCH(...) {
...
void QWidgetPrivate::setWinId(WId id)  // set widget identifier
{
  ...
  // to the mapper.
  bool userDesktopWidget =
      qt_desktopWidget != 0 && qt_desktopWidget != q && q->windowType() == Qt::Desktop;
    ...
    for (; i < w->d_func()->children.size(); ++i) {
      if (QWidget* sibling = qobject_cast<QWidget*>(w->d_func()->children.at(i))) {
        if (sibling->isVisible() && !sibling->isWindow()) {
          QRect siblingRect(ox + sibling->x(), oy + sibling->y(), sibling->width(),
                            sibling->height());
...
void QWidgetPrivate::subtractOpaqueSiblings(QRegion& sourceRegion,
                                            bool* hasDirtySiblingsAbove,
                                            bool alsoNonOpaque) const {
  Q_Q(const QWidget);
  static int disableSubtractOpaqueSiblings =
      qEnvironmentVariableIntValue("QT_NO_SUBTRACTOPAQUESIBLINGS");
...
void QWidgetPrivate::paintBackground(QPainter* painter, const QRegion& rgn, int flags) const {
  ...
  // If we are painting the viewport of a scrollarea, we must apply an offset to the brush in case
  // we are drawing a texture
...
/*!
\internal
Ensures that the widget has a window system identifier, i.e. that it is known to the windowing
system.
...
void QWidgetPrivate::inheritStyle() {
  ...
  QWidget* parent = q->parentWidget();
  QStyle* parentStyle =
      (parent && parent->d_func()->extra) ? (QStyle*)parent->d_func()->extra->style : 0;
...
bool QWidget::isEnabledTo(const QWidget* ancestor) const {
  const QWidget* w = this;
  while (!w->testAttribute(Qt::WA_ForceDisabled) && !w->isWindow() && w->parentWidget() &&
...
QRect QWidget::frameGeometry() const {
  ...
  if (isWindow() && !(windowType() == Qt::Popup)) {
    ...
    return QRect(data->crect.x() - fs.left(), data->crect.y() - fs.top(),
...
QSize QWidget::maximumSize() const {
  Q_D(const QWidget);
  return d->extra ? QSize(d->extra->maxw, d->extra->maxh) : QSize(QWIDGETSIZE_MAX, QWIDGETSIZE_MAX);
}
...
bool QWidgetPrivate::setMaximumSize_helper(int& maxw, int& maxh) {
  ...
  extra->explicitMaxSize =
      (maxw != QWIDGETSIZE_MAX ? Qt::Horizontal : 0) | (maxh != QWIDGETSIZE_MAX ? Qt::Vertical : 0);
  return true;
}
...
void QWidgetPrivate::resolveLayoutDirection() {
  ...
  if (!q->testAttribute(Qt::WA_SetLayoutDirection))
    setLayoutDirection_helper(q->isWindow() ? QApplication::layoutDirection()
                                            : q->parentWidget()->layoutDirection());
}
...
/*!
    ...
    widget. See the \l{Qt::CursorShape}{list of predefined cursor objects} for a range of useful
   shapes.
...
void QWidget::render(QPainter* painter,
  ...
  const QRegion toBePainted =
      !inRenderWithPainter ? d->prepareToRender(sourceRegion, renderFlags) : sourceRegion;
  ...
  if (painter->hasClipping()) {
    ...
    enginePriv->setSystemViewport(oldSystemClip.isEmpty() ? painterClip
                                                          : oldSystemClip & painterClip);
  } else {
...
static void sendResizeEvents(QWidget* target) {
  ...
  for (int i = 0; i < children.size(); ++i) {
    ...
    if (child->isWidgetType() && !child->isWindow() &&
        child->testAttribute(Qt::WA_PendingResizeEvent))
...
QPixmap QWidget::grab(const QRect& rectangle) {
  ...
  const QWidget::RenderFlags renderFlags =
...
QRegion QWidgetPrivate::prepareToRender(const QRegion& region, QWidget::RenderFlags renderFlags) {
  ...
  if (!isVisible && !isAboutToShow()) {
    ...
    if (topLevelExtra && !topLevelExtra->sizeAdjusted && !topLevel->testAttribute(Qt::WA_Resized)) {
      topLevel->adjustSize();
...
void QWidgetPrivate::drawWidget(QPaintDevice* pdev,
  ...
  if (graphicsEffect && graphicsEffect->isEnabled()) {
    ...
    QWidgetEffectSourcePrivate* sourced =
        static_cast<QWidgetEffectSourcePrivate*>(source->d_func());
    if (!sourced->context) {
      ...
      // Native widgets need to be marked dirty on screen so painting will be done in correct
      // context
      // Same check as in the no effects case below.
      if (backingStore && !onScreen && !asRoot &&
          (q->internalWinId() || !q->nativeParentWidget()->isWindow()))
...
// clip away the new area
      ...
      // context
      if (backingStore && !onScreen && !asRoot &&
          (q->internalWinId() || (q->nativeParentWidget() && !q->nativeParentWidget()->isWindow())))
      ...
      if (q->paintingActive())
        qWarning(
            "QWidget::repaint: It is dangerous to leave painters active on a widget outside of the "
            "PaintEvent");
...
void QWidgetPrivate::render(QPaintDevice* target,
  ...
  QRegion paintRegion =
      !inRenderWithPainter ? prepareToRender(sourceRegion, renderFlags) : sourceRegion;
...
/*!
    ...
    \l{http://developer.apple.com/documentation/UserExperience/Conceptual/OSXHIGuidelines/XHIGWindows/chapter_17_section_3.html}{proxy
   icon}
...
/*!
    ...
    proxy) if this widget or one of its parents is the \l{isActiveWindow()}{active window}. The \a
   reason argument will
...
void QWidget::setFocus(Qt::FocusReason reason) {
  ...
  if (f->isActiveWindow()) {
    ...
    if (!(testAttribute(Qt::WA_WState_Created) && window()->windowType() != Qt::Popup &&
          internalWinId()))
...
/*!
    ...
    If the widget has active focus, a \l{focusOutEvent()}{focus out event} is sent to this widget to
   tell it that it has
    lost the focus.
...
bool QWidget::isActiveWindow() const {
  ...
  if (style()->styleHint(QStyle::SH_Widget_ShareActivation, 0, this)) {
    if (tlw->windowType() == Qt::Tool && !tlw->isModal() &&
...
void QWidget::setTabOrder(QWidget* first, QWidget* second) {
  if (!first || !second || first->focusPolicy() == Qt::NoFocus ||
...
void QWidget::move(const QPoint& p) {
  ...
  if (testAttribute(Qt::WA_WState_Created)) {
    ...
    d->setGeometry_sys(p.x() + geometry().x() - QWidget::x(), p.y() + geometry().y() - QWidget::y(),
                       width(), height(), true);
    d->setDirtyOpaqueRegion();
  } else {
...
QByteArray QWidget::saveGeometry() const {
  ...
  // - Qt 4.8.6 - today, 5.4 - today: Version 2.0, save screen width in addition to check for high
  // DPI scaling.
...
#ifdef Q_DEAD_CODE_FROM_QT4_MAC
         << newFramePosition << newNormalPosition
#else
         << frameGeometry() << normalGeometry()
#endif  // Q_DEAD_CODE_FROM_QT4_MAC
         << qint32(screenNumber) << quint8(windowState() & Qt::WindowMaximized)
...
bool QWidget::restoreGeometry(const QByteArray& geometry) {
  ...
  stream >> restoredFrameGeometry >> restoredNormalGeometry >> restoredScreenNumber >> maximized >>
      fullScreen;
  ...
  if (!restoredNormalGeometry.isValid()) {
    ...
    restoredNormalGeometry.setSize(
        restoredNormalGeometry.size().expandedTo(d_func()->adjustedSize()));
  }
...
//   be possible to bring the size grip on screen by moving the window.
  ...
  restoredNormalGeometry.setHeight(
      qMin(restoredNormalGeometry.height(), availableGeometry.height() - frameHeight));
#endif

  if (!restoredFrameGeometry.intersects(availableGeometry)) {
    restoredFrameGeometry.moveBottom(
        qMin(restoredFrameGeometry.bottom(), availableGeometry.bottom()));
  ...
  if (!restoredNormalGeometry.intersects(availableGeometry)) {
    restoredNormalGeometry.moveBottom(
        qMin(restoredNormalGeometry.bottom(), availableGeometry.bottom()));
    restoredNormalGeometry.moveLeft(qMax(restoredNormalGeometry.left(), availableGeometry.left()));
    restoredNormalGeometry.moveRight(
        qMin(restoredNormalGeometry.right(), availableGeometry.right()));
  }
  restoredNormalGeometry.moveTop(
      qMax(restoredNormalGeometry.top(), availableGeometry.top() + frameHeight));
...
void QWidget::setContentsMargins(int left, int top, int right, int bottom) {
  Q_D(QWidget);
  if (left == d->leftmargin && top == d->topmargin && right == d->rightmargin &&
...
QRect QWidget::contentsRect() const {
  Q_D(const QWidget);
  return QRect(
      QPoint(d->leftmargin, d->topmargin),
...
void QWidgetPrivate::show_helper() {
  ...
  if (isWindow && !isEmbedded) {
    if ((q->windowType() == Qt::Tool) || (q->windowType() == Qt::Popup) ||
      ...
      q->raise();
      if (q->parentWidget() &&
          q->parentWidget()->window()->testAttribute(Qt::WA_KeyboardFocusChange))
...
void QWidgetPrivate::hide_helper() {
  ...
  isEmbedded = q->isWindow() && !bypassGraphicsProxyWidget(q) &&
               nearestGraphicsProxyWidget(q->parentWidget()) != 0;
#else
  Q_UNUSED(isEmbedded);
#endif
...
void QWidgetPrivate::hideChildren(bool spontaneous) {
  ...
  for (int i = 0; i < childList.size(); ++i) {
    ...
    if (!widget || widget->isWindow() || widget->testAttribute(Qt::WA_WState_Hidden))
      continue;
#ifdef Q_DEAD_CODE_FROM_QT4_MAC
    // Before doing anything we need to make sure that we don't leave anything in a non-consistent
    // state.
    ...
    // The solution is simple, before going through with this we check if there are any mouse_down
    // events in
    // progress, if so we check if it is related to this widget or not. If so, we just reset the
    // mouse_down and
    // then we continue.
    // In X11 and Windows we send a mouse_release event, however we don't do that here because we
    // were already
    // ignoring that from before. I.e. Carbon did not send the mouse release event, so we will not
    // send the
    // mouse release event. There are two ways to interpret this:
    // 1. If we don't send the mouse release event, the widget might get into an inconsistent state,
    // i.e. it
    // might be waiting for a release event that will never arrive.
    // 2. If we send the mouse release event, then the widget might decide to trigger an action that
    // is not
...
bool QWidgetPrivate::close_helper(CloseMode mode) {
  ...
  if (quitOnClose) {
    ...
    if (lastWindowClosed) {
      ...
      QCoreApplicationPrivate* applicationPrivate =
  ...
  switch (event->type()) {
    ...
    case QEvent::KeyPress: {
      ...
      if (!k->isAccepted() && k->modifiers() & Qt::ShiftModifier && k->key() == Qt::Key_F1 &&
          d->whatsThis.size()) {
        QWhatsThis::showText(mapToGlobal(inputMethodQuery(Qt::ImCursorRectangle).toRect().center()),
                             d->whatsThis, this);
        k->accept();
      }
#endif
    ...
    case QEvent::ContextMenu:
      switch (data->context_menu_policy) {
        ...
        case Qt::ActionsContextMenu:
          if (d->actions.count()) {
            QMenu::exec(d->actions, static_cast<QContextMenuEvent*>(event)->globalPos(), 0, this);
            break;
          }
// fall through
#endif
    ...
    case QEvent::WindowStateChange: {
      const bool wasMinimized =
          static_cast<const QWindowStateChangeEvent*>(event)->oldState() & Qt::WindowMinimized;
    ...
    case QEvent::ToolTip:
      if (!d->toolTip.isEmpty())
        QToolTip::showText(static_cast<QHelpEvent*>(event)->globalPos(), d->toolTip, this, QRect(),
                           d->toolTipDuration);
      else
        event->ignore();
      break;
...
void QWidget::changeEvent(QEvent* event) {
  switch (event->type()) {
    ...
    case QEvent::ThemeChange:
      if (QApplication::desktopSettingsAware() && windowType() != Qt::Desktop && qApp &&
...
void QWidget::setLayout(QLayout* l) {
  ...
  if (oldParent && oldParent != this) {
    if (oldParent->isWidgetType()) {
      ...
      qWarning(
          "QWidget::setLayout: Attempting to set QLayout \"%s\" on %s \"%s\", when the QLayout "
...
/*!
    ...
    Returns \c true if the widget's preferred height depends on its width; otherwise returns \c
   false.
*/
...
QWidget* QWidgetPrivate::childAtRecursiveHelper(const QPoint& p,
  ...
  for (int i = children.size() - 1; i >= 0; --i) {
    QWidget* child = qobject_cast<QWidget*>(children.at(i));
    if (!child || child->isWindow() || child->isHidden() ||
...
void QWidgetPrivate::updateGeometry_helper(bool forceUpdate) {
  ...
  if (forceUpdate || !extra || extra->minw != extra->maxw || extra->minh != extra->maxh) {
    const int isHidden =
        q->isHidden() && !size_policy.retainSizeWhenHidden() && !retainSizeWhenHiddenChanged;
...
void QWidgetPrivate::setWindowFlags(Qt::WindowFlags flags) {
  ...
  if ((q->data->window_flags | flags) & Qt::Window) {
    ...
    // for backward-compatibility we change Qt::WA_QuitOnClose attribute value only when the window
    // was recreated.
    adjustQuitOnCloseAttribute();
  } else {
...
void QWidget::setParent(QWidget* parent, Qt::WindowFlags f) {
  ...
  if (newParent && parent && !desktopWidget) {
    ...
    else if (parent->d_func()->nativeChildrenForced() ||
...
void QWidgetPrivate::setParent_sys(QWidget* newparent, Qt::WindowFlags f) {
  ...
  if (parent != newparent) {
    QObjectPrivate::setParent_helper(
        newparent);  //### why does this have to be done in the _sys function???
    if (q->windowHandle()) {
  ...
  bool explicitlyHidden =
      q->testAttribute(Qt::WA_WState_Hidden) && q->testAttribute(Qt::WA_WState_ExplicitShowHide);

  // Reparenting toplevel to child
  if (wasCreated && !(f & Qt::Window) && (oldFlags & Qt::Window) &&
...
void QWidget::setAttribute(Qt::WidgetAttribute attribute, bool on) {
  ...
  Q_D(QWidget);
  Q_STATIC_ASSERT_X(
      sizeof(d->high_attributes) * 8 >= (Qt::WA_AttributeCount - sizeof(uint) * 8),
      "QWidget::setAttribute(WidgetAttribute, bool): "
      "QWidgetPrivate::high_attributes[] too small to contain all attributes in WidgetAttribute");
#ifdef Q_OS_WIN
  // ### Don't use PaintOnScreen+paintEngine() to do native painting in some future release
  if (attribute == Qt::WA_PaintOnScreen && on && windowType() != Qt::Desktop &&
      !inherits("QGLWidget")) {
  ...
  // Don't set WA_NativeWindow on platforms that don't support it -- except for QGLWidget, which
  // depends on it
  ...
  switch (attribute) {
    ...
    case Qt::WA_DropSiteRegistered: {
      ...
      for (int i = 0; i < d->children.size(); ++i) {
        QWidget* w = qobject_cast<QWidget*>(d->children.at(i));
        if (w && !w->isWindow() && !w->testAttribute(Qt::WA_AcceptDrops) &&
...
#ifdef Q_DEAD_CODE_FROM_QT4_MAC
      d->setStyle_helper(style(), false,
                         true);  // Make sure things get unpolished/polished correctly.
    ...
    case Qt::WA_ShowModal:
      if (!on) {
        ...
        data->window_modality =
            (w && w->testAttribute(Qt::WA_GroupLeader)) ? Qt::WindowModal : Qt::ApplicationModal;
...
QWidget* QWidgetPrivate::widgetInNavigationDirection(Direction direction) {
  ...
  const int sourceX = (direction == DirectionNorth || direction == DirectionSouth)
                          ? (sourceRect.left() + (sourceRect.right() - sourceRect.left()) / 2)
                          : (direction == DirectionEast ? sourceRect.right() : sourceRect.left());
  const int sourceY = (direction == DirectionEast || direction == DirectionWest)
  ...
  foreach (QWidget* targetCandidate, QApplication::allWidgets()) {
    const QRect targetCandidateRect =
        targetCandidate->rect().translated(targetCandidate->mapToGlobal(QPoint()));

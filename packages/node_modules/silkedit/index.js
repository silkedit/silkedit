...
module.exports = {
	...
	commands: {
		...
		,"new_package": () => {
      var validate;
      fs.readFile(silk.Constants.userPackagesJsonPath, 'utf-8', (err, data) => {
        var packages
        if (err) {
          validate = null
        } else {
        ...
        const pkgName = silk.showInputDialog("Enter new package path", "my_package", validate)
  		  ...
  		  const pkgPath = path.normalize(silk.Constants.userPackagesNodeModulesPath + '/' + pkgName)
   					  ...
   					  replaceFileContent(pkgPath + "/index.js", /<name>/g, path.basename(pkgPath), (err) => {
   					    ...
   					    fs.readFile(newPkgJsonPath, (err, data) => {
    					    ...
    					    packages.push(JSON.parse(data))
     					    fs.writeFile(silk.Constants.userPackagesJsonPath, JSON.stringify(packages), (err) => {
     					      if (err) throw err

...
module.exports = {
	...
	commands: {
		...
		,"open_folder": () => {
			const path = DialogUtils.showDirectoryDialog('Open Folder')
			if (path != null) {
				silkedit.ProjectManager.open(path)
			}
		}
		...
		,"show_scope": () => {
			...
			if (win != null && editView != null) {
				win.statusBar().showMessage(editView.scopeName(), 1000);
			}
		}
		,"show_scope_tree": () => {
			const win = silkedit.App.activeWindow()
      ...
      fs.readFile(silkedit.Constants.userPackagesJsonPath, 'utf-8', (err, data) => {
        ...
        const dialog = new InputDialog("Enter new package path", "my_package", validate);

...
function move_cursor(args) {
  const editView = silkedit.activeTextEditView()
...
module.exports = {
	...
	commands: {
		"new_file": () => {
			const tabView = silkedit.activeTabView()
			if (tabView != null) {
				tabView.addNew();
			}
		}
		,"open": () => {
			const paths = silkedit.showFileAndFolderDialog('Open')
			paths.forEach(function(path) {
				silkedit.open(path)
			})
		}
		,"open_file": () => {
			const paths = silkedit.showFilesDialog('Open File')
			paths.forEach(function(path) {
				silkedit.open(path)
			})
		}
		,"open_folder": () => {
			const path = silkedit.showFolderDialog('Open Folder')
			if (path != null) {
				silkedit.open(path)
			}
		}
		...
		,"save_all": () => {
			const tabViewGroup = silkedit.activeTabViewGroup()
		...
		,"close_tab": () => {
			...
			if (tabView != null) {
				if (tabView.count() > 0) {
					...
					const win = silkedit.activeWindow()
					if (win != null) {
						win.close()
					}
				}
      ...
      fs.readFile(silkedit.Constants.userPackagesJsonPath, 'utf-8', (err, data) => {
        ...
        const pkgName = silkedit.showInputDialog("Enter new package path", "my_package", validate)
  			// copy hello example package to a new package directory
  			if (pkgName == null) return
				
  		  const pkgPath = path.normalize(silkedit.Constants.userPackagesNodeModulesPath + '/' + pkgName)
    					    ...
    					    packages.push(JSON.parse(data))
     					    fs.writeFile(silkedit.Constants.userPackagesJsonPath, JSON.stringify(packages), (err) => {
     					      if (err) throw err
       						  ...
       						  silkedit.open(pkgPath)
       						  silkedit.open(pkgPath + '/package.json')
     					    });
   					    });
   					  });
   					});
